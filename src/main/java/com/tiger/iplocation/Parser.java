...
/**
 * com.tiger.iplocation.Parser
 *
 * @author: zhaolihe
 * @email: dayingzhaolihe@126.com
 * @date 2018-07-16 11:13:00
 */
public class Parser {

    // 整型255
    private final static int MAX_TOKEN = 0xFF;
    ...
    public Parser(String dbFile) throws FileNotFoundException {
     ...
     */
    public Parser(DbConfig dbConfig, String dbFile ) throws FileNotFoundException
    {
    ...
    public DataBlock memorySearch(long ip) throws IOException
    {
        ...
        long startIP, endIP, dataptr = 0;
        while ( l <= h ) {
            int m = (l + h) >> 1;
            int p = (int)(firstIndexPtr + m * blen);

            startIP = Util.getIntLong(dbBinStr, p);
            if ( ip < startIP ) {
                h = m - 1;
            } else {
                endIP = Util.getIntLong(dbBinStr, p + 4);
                if ( ip > endIP ) {
                    l = m + 1;
                } else {
        ...
        //get the data
        int dataLen = (int)((dataptr >> 24) & MAX_TOKEN);
    ...
    public DataBlock getByIndexPtr( long ptr ) throws IOException
    {
        ...
        int dataLen = (int)((extra >> 24) & MAX_TOKEN);
    ...
    public DataBlock btreeSearch( long ip ) throws IOException
    {
        ...
        String region = new String(data, 0, data.length , "UTF-8");
